...
function applyStyles(element: HTMLElement, styles: any) {
  ...
  if (styles.bgborder)
    element.style.border = `${styles.bgborder.top}px solid ${hexToRgba(styles.bgborder.color, styles.bgborder.alpha)}`;
...
export function buildLoginPage(settings: any): { [id: string]: HTMLElement } {
  ...
  elements.identification = createInputGroup(
    innerData.identification,
